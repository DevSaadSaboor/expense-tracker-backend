...
from app.service.categories_service import (
    ...
    service_get_category_by_id,
    service_get_categories_by_user,
)
...
def create_category_endpoint(
    payload: CategoriesCreateRequest, user_id: int = Depends(get_current_user_id)
):
    try:
        category = service_create_category(user_id=user_id, name=payload.name)
...
def get_category_by_id_endpoint(
    category_id: int, user_id: int = Depends(get_current_user_id)
):
...
def get_update_endpoint(
    category_id: int,
    payload: CategoriesUpdateRequest,
