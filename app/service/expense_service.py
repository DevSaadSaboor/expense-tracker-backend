...
from app.storage.expenses_repo import (
    get_monthly_totals_between_dates,
...
def service_create_expense(user_id, category_id, amount, note, spend_at):
    ...
    new_create_expense = create_expense(
        user_id=user_id,
        category_id=category_id,
        amount=amount,
        note=note,
...
def service_update_expense(user_id, expense_id, fields: dict):
    ...
    update_expenses = update_expense(
        expense_id=expense_id, user_id=user_id, fields=new_field
    )
    logger.info("expense_updated", user_id=user_id, expense_id=expense_id)
...
def service_delete_expense(user_id, expense_id):
    ...
    if not expense:
        logger.warning(
            ...
            reason="invalid expense",
            user_id=user_id,
            expense_id=expense_id,
        )
        raise InvalidUserInput("Expense does not found")
    delete_expense(expense_id, user_id)
    logger.info("expense_deleted", user_id=user_id, expense_id=expense_id)
    return True
...
def service_get_monthly_totals_between_dates(user_id, start_date, end_date):
    ...
    if not is_valid_date(end_date):
        raise InvalidUserInput("Invalid date format")
    monthly_total_with_dates = get_monthly_totals_between_dates(
